...
void display(void)   // Creamos la funcion donde se dibuja
{
	...
	glTranslatef(0, 4, movKit);		//movkit es el movimiento
									//Pongo aquí la carroceria del carro			Glrender mandar a dibujar lo que ya cargamos
	...
	pintaTexto(-12, 12.0, -20, (void *)font, "Gustaf Bermudez Sotelito");
	glColor3f(1.0, 1.0, 1.0);
	glEnable(GL_LIGHTING);
...
void animacion()
{
	...
	{
		if (movKit < -120)
			g_fanimacion = true; //cuando la animacion sea 120, activamos MovKit en sentido contrario
		movKit -= 0.5;
