...
//*************		Practica 12: Animacion con maquinas de estado							******//
...
//Animación del coche
float angRot = 0.0;
float movKitX = 0.0;		//direccion de mov
float movKitZ = 0.0;
float rotKit = 0.0;			//rotacion
float rotTires = 0.0;
bool g_fanimacion = false;
bool g_avanza = false;

bool circuito = false;		//banderas se usan para activar un determinado movimiento
...
void animacion()
{
	...
	if (circuito)		//comienza maquina de estados
	{
		...
		{
			movKitZ++;
			if (movKitZ > 155)				//cuando se llega a 155 
			{
				recorrido1 = false;		//el recorrido 1 es falso y se desactiva
				recorrido2 = true;		//el recorrido 2 se activa
			}
		}
		if (recorrido2)		//estado B
		{
			rotKit = 90;			//rotacion de 90°
			movKitX++;				//avanza
			if (movKitX > 125)		//se debe llegar a la condicion de 125
			{
				recorrido2 = false;	//recorrido 2 se desactiva
				recorrido3 = true;	//recorrido 3 s activca

			}
		}
		if (recorrido3)		//estado C
		{
			//rotKit = 180;		//con 180 grados sigue el recorrdio
			rotKit = 220;		//	rotacion del carrito para avanzar
			movKitZ -= 2.5;		// 
			movKitX--;
...
int main(int argc, char** argv)   // Main Function
{
	...
	glutCreateMenu(menu);
	glutAddSubMenu("Automovil Loco", submenu);
